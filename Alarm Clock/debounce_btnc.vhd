...
architecture Behavioral of debounce_btnc is
    ----------------------------------------------------------------
    -- Constants
    ----------------------------------------------------------------
...
begin
    ...
    begin
        if rising_edge(clk) then
            if rst = '1' then
                ...
                -- Input synchronizer
                sync1 <= sync0;
                sync0 <= btnc_in; -- Opraveno na btnu_in
    ...
    -- Outputs
    ----------------------------------------------------------------
    btnc_state <= debounced; -- Opraveno na btnu_state

    -- One-clock pulse when button pressed
    btnc_press <= debounced and not(delayed);    -- Opraveno na btnu_press
    btnc_release <= delayed and not(debounced);  -- Opraveno na btnu_release
