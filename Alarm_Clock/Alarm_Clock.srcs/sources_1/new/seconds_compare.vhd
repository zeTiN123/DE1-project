...
entity seconds_compare is
    Port ( rst : in STD_LOGIC;
           ...
           alarm_en : in std_logic;     --sw(1)     1 znamena odemknuto
           buzzer_off : in STD_LOGIC; -------pripojeno na signal od press_btnD 
           s_clock : in STD_LOGIC_vector (16 downto 0); ---aktualni pocet sekund z cloku
           s_alarm : in STD_LOGIC_vector (16 downto 0); ---prepocteny sekundy z budiku pro compare
...
architecture Behavioral of seconds_compare is
    ...
    signal sig_compare_lock : std_logic := '1'; ---zamek aby na zacatku před nastavenim nezvonil
...
begin
    ...
    begin
        if rising_edge(clk) then    ---detekuje nabeznou hranu
            if (rst = '1') then       ---synchronizace
                ...
                sig_compare_lock <= '1'; ---po zapnuti jsme zamkli v nastaveni
                
            elsif (buzzer_off = '1') then  ---zmacknuti BTNC vypne buzzer
                sig_compare_lock <= '1';    ---odemkne pro compare
                sig_buzzer_activate <= '0'; 
                    
            elsif (s_clock = s_alarm and alarm_en = '1' and sig_compare_lock = '1') then  ---pokud se hodnoty sekund shoduji zapne se buzzer
    ...
    buzzer_interval <= sig_buzzer_activate and s_clock(0);  --sig_frequent_buzz
