...
architecture Behavioral of debounce_btnu is
    ----------------------------------------------------------------
    -- Constants
    ----------------------------------------------------------------
...
begin
    ...
    begin
        if rising_edge(clk) then
            if rst = '1' then
                ...
                -- Input synchronizer
                sync1 <= sync0;
                sync0 <= btnu_in; -- Opraveno na btnu_in
    ...
    -- Outputs
    ----------------------------------------------------------------
    btnu_state <= debounced; -- Opraveno na btnu_state

    -- One-clock pulse when button pressed
    btnu_press <= debounced and not(delayed);    -- Opraveno na btnu_press
    btnu_release <= delayed and not(debounced);  -- Opraveno na btnu_release
